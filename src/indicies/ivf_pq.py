...
class IVFPQIndexer(object):
    ...
    def __init__(self, 
        ...
        if os.path.exists(index_path) and os.path.exists(self.meta_file):
            ...
            with open(self.meta_file, "rb") as f:
                self.custom_id_to_metadata = pickle.load(f)
    ...
    def _add_keys(self, index_path, trained_index_path):
        ...
        for shard_id, embed_path in enumerate(self.embed_paths[start_shard:], start_shard):
            ...
            for s in range(0, n, bs):
                ...
                t4 = time.time()
                print(f"Shard {shard_id}: Load pkl={t1-t0:.2f}s, Slice IDs={t2-t1:.2f}s, Update metadata time={t4-t2:.2f}s")
    ...
    def build_passage_pos_id_array(self, ):
        ...
        target_jsonl_files = [
            os.path.basename(p).replace('.pkl', '.jsonl') for p in self.embed_paths
        ]
        passage_pos_ids, passage_filenames = get_passage_pos_ids(self.passage_dir, self.pos_array_save_path, 
                                                                 self.passage_filenames_save_path, self.deprioritized_domains, start_shard=self.manual_start, end_shard=self.manual_end)
        return passage_pos_ids, passage_filenames

    def load_psg_pos_id_array(self,):
        print(f"Loading passage position ID array from {self.pos_array_save_path} and filenames from {self.passage_filenames_save_path}...")
        print("exists:", os.path.exists(self.pos_array_save_path), os.path.exists(self.passage_filenames_save_path))
        if os.path.exists(self.pos_array_save_path) and os.path.exists(self.passage_filenames_save_path):
            with open(self.pos_array_save_path, 'rb') as f:
                psg_pos_id_array = np.load(f, allow_pickle=True)
    ...
    def _get_passage(self, custom_id):
        ...
        position = self.psg_pos_id_array[shard_id][local_index]
        with open(os.path.join(self.passage_dir, filename), 'r') as file:
            file.seek(position)
            line = file.readline()
        return json.loads(line)

    def _get_domain(self, custom_id: int):
        if custom_id not in self.custom_id_to_metadata:
            return "unknown_domain"
        
        shard_id, _ = self.custom_id_to_metadata[custom_id]
        filename = self.passage_filenames[shard_id]
        return os.path.basename(filename).split("raw_passages")[0].split("--")[0]

    def get_retrieved_passages(self, all_custom_ids):
        domains, passages, db_ids = [], [], []
        for query_custom_ids in all_custom_ids:
            domain_per_query = [self._get_domain(int(custom_id)) for custom_id in query_custom_ids]
            passages_per_query = [self._get_passage(int(custom_id))["text"] for custom_id in query_custom_ids]
            db_ids_per_query = [int(custom_id) for custom_id in query_custom_ids]
    ...
    def search(self, query_embs, k=4096, id_range_min=None, id_range_max=None):
        query_embs_float32 = query_embs.astype(np.float32)
        if id_range_min is not None and id_range_max is not None:
            print(f"Searching with ID range filter: [{id_range_min}, {id_range_max-1}]")
            selector = faiss.IDSelectorRange(id_range_min, id_range_max)
            params = faiss.SearchParametersIVF()
            params.sel = selector
            params.nprobe = self.index.nprobe
            all_scores, all_custom_ids = self.index.search(query_embs_float32, k, params=params)
        else:
            all_scores, all_custom_ids = self.index.search(query_embs.astype(np.float32), k)
        all_domains, all_passages, db_ids = self.get_retrieved_passages(all_custom_ids)
